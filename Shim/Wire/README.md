...
The **Panel** dropdown includes a `<None>` option so circuits that never belong on a panel (DMX/DALI, etc.) can be created unassigned in one step — picking it disconnects any auto-assigned panel via `ElectricalSystem.DisconnectPanel()`. The dropdown defaults to the last circuit's choice (a panel, or `<None>` if the previous one was left unassigned); see [Circuit Creation](#circuit-creation).
