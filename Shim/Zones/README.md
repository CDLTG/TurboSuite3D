...
Three-tab modeless utility: circuit load names, dimmer-panel allocation, and DALI loop declaration. The window stays open while you work in Revit — pan, zoom, select, and run other commands without closing it. All Revit writes go through an `IExternalEventHandler` (see CLAUDE.md "Modeless pattern").
...
Declare **DALI loops** and place their modules. A loop is a named grouping of `Control Zone` values sharing one DALI bus (one `LQSE2-1DALUNV-D`): the tab pools the unassigned Control Zones, groups them into named loops, and requires a **ZONE** per loop — the panel its module sits in. DALI circuits are created unassigned (like DMX), so the zone can't be derived and the designer picks it. Deliberately barren — a future TurboDALI owns solve visualization and numbering.

Loops persist in `DaliStorageService` (`PayloadVersion` 2 carries each loop's `AssignedZone`); the placement map is read **once at window open** and fed to the Panel Breakdown, so edits reflect on reopen (no live cross-tab wiring — TurboZones is not open alongside the DALI programming in practice). The tab warns on loops left unassigned (still ordered, just not placed) and on loops over the **64-loads/bus** cap. In the Panel Breakdown a placed DALI slot renders pastel purple and shows **loads / 64** (red when over one bus).
