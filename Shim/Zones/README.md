...
- **DMX warnings never fail a document.** A design that will not solve, or one solved over partly zoned tape, contributes a warning line naming the reason — including *"N DMX fixtures have no Control Zone assigned"*, which is otherwise silent now that DMX circuits are excluded from Unassigned.
