...
| `TurboSuite.Mask` | TurboMask — masking region + per-fixture annotation stamps (extracted from each fixture family's nested Generic Annotation, loaded as `Stamp_*`), grouped per run; shipped (ribbon still uses the `Blank` placeholder icon pending a dedicated one) |
