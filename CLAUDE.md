...
Platform target is **x64**. All Revit-coupled add-in source lives **once** in `Shim/` (a Visual Studio Shared Project — `Shim/Shim.projitems` imported by both csprojs; `Shim/Shim.shproj` is the VS node, never built by the CLI). It compiles into **two thin per-version shims** — `Revit2025/TurboSuite.Revit2025.csproj` (net8.0-windows, Revit 2025 API) and `Revit2024/TurboSuite.Revit2024.csproj` (net48, Revit 2024 API) — each of which emits `TurboSuite.dll` via `AssemblyName` into its own `Addins\{year}\` folder. **The shared source carries no version constant: the Revit year comes from the running Revit at runtime** (`UIControlledApplication.ControlledApplication.VersionNumber`, captured in `OnStartup`), so compile-time divergence is confined to the csproj TFM/API refs plus two narrow seam patterns. **(1) Single shared file** — `ElementRefConversions` (the `.Value`↔`.IntegerValue` boundary): one file in `Shim/` that compiles for both because the member exists in each API (just differently typed). **(2) Per-shim split file** — `Revit{year}/Setup/LinkGraphicsSeam.cs`: the same namespace + class declared once under `Revit2024/` and once under `Revit2025/`, each picked up only by its own shim's default SDK globbing, so shared code can call it while each version supplies its own body. Use (2) when an API member exists in *only one* version and would otherwise break the other shim's build — e.g. the Revit 2025-only RVT link *Custom* display settings (`RevitLinkGraphicsSettings.ObjectStyles`/`ViewRange`/…), which the 2024 file stubs out. Supporting these are the version-agnostic, multi-targeted (`net48;net8.0-windows`) `Core/TurboSuite.Core.csproj` and `Abstractions/TurboSuite.Abstractions.csproj` (no Revit refs), plus `Updater/TurboSuiteUpdater.csproj` (auto-update helper, also multi-targeted) and `Installer/TurboSuiteInstaller.csproj` (standalone WPF installer). Automated tests live in `Tests/TurboSuite.Core.Tests.csproj` (xUnit, net8.0-windows; run `dotnet test`) — currently the pure TurboDMX `Core/Dmx/` oracle suite (engine + numbering-lock reconciler + placement planner + zone/cluster builder + ViewModel; ~161 tests, the suite's only tests; the Revit-coupled shims are validated by manual in-Revit testing). There are no linting configurations.
...
| `TurboSuite.Driver` | TurboDriver (deploys power supplies, modal) + TurboRPS (staleness dashboard + batch in-place driver-type corrector, MVVM modeless; Core VMs `RpsMainViewModel`/`RpsCircuitRowViewModel` + `StaleClassifier` + `RpsDeferral`, shim `RpsRevitOperations`/`RpsCircuitDataBuilder`/`RpsDeferralStorageService`. Right-click **Defer circuit** flags an intentionally-"wrong" config — stored in ExtensibleStorage **on the circuit element** (per-element entity, not the doc-singleton pattern), masking its verdict to DEFERRED and resurfacing as REVIEW if the snapshotted config later drifts) |
...
| `TurboSuite.Mask` | TurboMask — masking region + per-fixture annotation stamps (extracted from each fixture family's nested Generic Annotation, loaded as `Stamp_*`) + detail-line overlays of wires connected to the masked devices (real wires left connected/hidden under the mask — overlays are view-only stand-ins drawn on top, never a delete-recreate, so connectivity is untouched), grouped per run; shipped (ribbon still uses the `Blank` placeholder icon pending a dedicated one) |
...
| `TurboSuite.Dmx` | TurboDMX — DMX-controlled RGBW LED tape/fixture automation (decoder/driver packing, addressing, one-line). **Gated behind `ExperimentalCommandsEnabled`; mid-build (Phases 0–3 done, Phase 4 one-line pending).** Pure engine + VMs in `Core/Dmx/` (ported from a standalone spike, unit-tested in `Tests/`); Revit-coupled half in `Shim/Dmx/`. A modeless window (`DmxMainViewModel`) reads DMX fixtures (`DMX Channels` > 0) grouped by the `Control Zone` instance param, discovers decoder/driver family types, and shows a live "bill" off the pure `DmxSolver`. **The window is loop-centric:** a multi-select **zone pool** (unassigned Control Zones — the engine auto-packs these, so the pool is the loop-level residual) feeds a **loops tree** (each declared loop owns its assigned zones, and each zone nests its own cluster sub-builder). Zones move pool→loop via destination-owned buttons (`+ Add` on a loop / `+ New loop from selection`; `←` returns a zone to the pool) — enforcing one-zone-per-loop structurally. A single **Refresh** re-reads the model **and** re-solves; the bill otherwise auto-solves (`Run()`) on every declaration change, so there's no separate Run button. Declared loops are still the engine's `LoopDeclaration` (geometry override on top of auto-pack); pooled zones fall through to auto-packing. **Model writes (Phase 2):** **per-loop click-to-place** — each loop row carries its own `Place ▸` (one pick lands just that loop's decoders+drivers) plus a placement-state glyph (●/◑/○) derived from the numbering + the persisted registry (no model scan); `IDmxPlacementService.Place` takes an `onlyInterfaceNumber` filter to scope a Place to one interface while orphan validity stays computed from the whole solve. Devices are decoder+driver families (`DmxPlacementService` / `DmxPlacementPlanner`), writing decoder `Switch ID` (`DEC n`) + auto-syncing tags (Switch-ID on decoders via `AL_Tag_Lighting Device (SwitchID)`, Type-Mark on drivers via `AL_Tag_Lighting Device (Type)` — driver Type Mark is tagged, not written). No wiring/circuits (the §0c feed half is deferred). **Numbering lock (Phase 3, §8c):** Control-Zone-anchored snapshot baseline (`DmxLockReconciler` in `Core/Dmx/Lock/`) — Unlocked re-numbers freely; Locked pins issued DEC #s, appends additive decoders past the high-water mark, and flags decoder-type/interface drift as **REVIEW** (never a silent renumber). Two-button Lock/Unlock (Lock re-labels to "Re-lock" once Locked = re-baseline without renumbering). **Placement is non-destructive** (Option A): re-Place lands only the unbuilt remainder and removes orphans (decoders whose DEC # left the solve) + their paired driver, via a model-scan + a persisted placement registry (`DmxModuleState.Placed`, decoder/driver ids) — auto when Unlocked, confirmed when Locked. **Cluster sub-builder (§8d):** selection-driven per-zone split for location-spanning zones (`DmxZoneBuilder.Build(fixtures, clusters)` partitions a zone by fixture ElementId with a visible `(unclustered)` residual; `IDmxModelSelection` reads/highlights the model); flat one-cluster-per-zone default otherwise. The cluster UI now **nests inside each zone node in the loops tree** (only zones with ≥2 runs show it), not a separate section. Doc-side state (settings/curated-pool ticks/loops/clusters/placement-registry/lock-snapshot) persists in one JSON-backed ExtensibleStorage schema (`DmxStorageService`, payload DTOs in `Core/Dmx/Persistence/`, coalesced writes via `DmxStatePersister`). Schema conventions centralized in `Core/Dmx/DmxSchemaAssumptions.cs`. **One-line (Phase 4) — built:** the decoupled native-Revit diagram is **per-loop, view-owned** — each loop owns a Drafting View (deterministic name + persisted view id in `DmxModuleState.OneLineViews`); a draw is a **wipe-and-redraw** from the solve snapshot, never a hand-edit. Pure half in `Core/Dmx/OneLine/` (`DmxOneLineGeometry` = the authored 6-family spec — names/box sizes/connection offsets; `DmxOneLinePlanner` → one `DmxOneLineDrawing` per loop: interface+processor top, `[driver][decoder][homerun]` rows grouped into `120V FEED` blocks from `InterfaceSolution.Feeds`, DMX daisy chain top→bottom to a terminator, ① ③ ⑥ ⑦ + homerun-gauge markers off the wire legend, single homerun leg + `REFER TO PLAN` header, tape type not drawn per §8a; unit-tested). Revit half = `Shim/Dmx/Services/DmxOneLineService.cs` (per-loop `One-line ▸` in the loops tree). **§0c breakers were reconciled to make the count match the drawing:** `BreakerPacker.Pack` is now order-preserving **next-fit run per interface** (feeds are consecutive DEC#s, never span interfaces), so `bill.RequiredBreakers` == the drawn `120V FEED` blocks. **Drawn correctly in-Revit on a real model** (2026-06-29); **owed:** a cosmetic polish pass (XAML + `DmxOneLineGeometry.Layout` constants), family-text verification (driver type-mark, processor/terminator static text), and the author-once wire-legend artifact (static ①–⑦ table, not generated, §8a). Design docs are local-only under `Specs/_DMX/` (gitignored; `TurboDMX-BuildPlan.md` is the canonical entry point) |
| `TurboSuite.Snoop` | TurboSnoop — read-only "which VG checkbox do I uncheck?" reporter: pick a linked arch family, list the Visibility/Graphics **Category → Subcategory** checkboxes its geometry draws under, split into Model geometry (one viewless `get_Geometry`) vs view-dependent/annotation (per-`ViewPlan` sweep + union — annotation is visibility-filtered, so a single view misses it). Reaches non-shared nested annotation by reading `GraphicsStyleId` off rendered geometry (no reference needed). Modeless WPF TreeView (`TurboSnoopWindow` + Core `SnoopMainViewModel`/`SnoopNodeViewModel`): pick-first flow (run → `PickObject` → window; Escape cancels before anything opens), family name as header + family category as root to mirror the VG menu, leaf checkboxes bulleted. Deliberately **read-only, no Apply** — no API can flip the per-link VG checkbox (`RevitLinkGraphicsSettings` has no per-category setter; `Custom` is unsettable in 2024), so it names the box and the user unchecks it, with the modeless window keeping Revit's VG/VV keybind live (`ShowActivated=false`). Shipped (ribbon still uses the `Blank` placeholder icon pending a dedicated one). Design/rejected-alternatives rationale in `Shim/Snoop/` headers |
| `TurboSuite.Schedule` | TurboSchedule — page-per-Type-Mark form-view spec editor for lighting fixtures (`OST_LightingFixtures`) and drivers (`OST_LightingDevices`), unifying the two native spec schedules. Modeless WPF (`TurboScheduleWindow` + Core `ScheduleMainViewModel`); data-driven `FieldDef` roster filtered by `PageKind` (Fixture/Driver) renders one ordered field list across Identity/Electrical/Mechanical/Photometric/Notes sections. `ScheduleTypeCollector` reads+reconciles each field across all symbols sharing a Type Mark into a `SpecField` with four states — n/a (param absent on a symbol, checked first) / read-only (`IsReadOnly`) / ⟨varies⟩ (symbols disagree) / normal; Core can't reference Revit `StorageType`, so the live storage type is mapped to a Core `SpecValueKind` for display while the writer re-reads it at save (a Yes/No Integer param brands as `Boolean` and renders as a checkbox; the writer sets Integer params via `Parameter.Set(int)`, not `SetValueString`). Unit-bearing Doubles display through `AsValueString`, so units and rounding follow Project Units settings, not app code. Explicit Save (dirty tracking, Option A) flushes every dirty page in one transaction via `ScheduleWriterService`/`RevitWorkQueue` (re-resolves symbols by Type Mark; `SetValueString` failures skipped+reported, left dirty). In-app copy/paste (`SpecClipboard`, whole-type same-kind + per-section cross-kind). Shipped (ribbon still uses the `Blank` placeholder icon pending a dedicated one) |
