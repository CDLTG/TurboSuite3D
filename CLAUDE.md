...
TurboSuite is a unified Autodesk Revit add-in for electrical/lighting automation, written in C#, supporting **Revit 2024, 2025, and 2026**. A per-version `TurboSuite.dll` (.NET Framework 4.8 for 2024, .NET 8.0-windows for 2025, .NET 10.0-windows for 2026) implements `IExternalApplication` and registers sixteen shipped commands (TurboDriver, TurboRPS, TurboName, TurboBubble, TurboTag, TurboWire, TurboZones, TurboNumber, TurboCompact, TurboTab, TurboDocs, TurboMask, TurboSnoop, TurboSchedule, TurboSetup, TurboNudge) plus a Settings dialog across four ribbon panels (Settings, Tools, Commands, Utilities). Two more commands, TurboDMX and TurboDALI, are compiled in but gated behind `ExperimentalCommandsEnabled` in `App/TurboSuiteApplication.cs`.
...
| `TurboSuite.Nudge` | TurboNudge — slides a point-based family (keypad) along its wall to exactly 5" from a picked corner. Point-keyed: reads only the family's `BasisX` direction and the picked `XYZ` — no host/line lookup, so hosted (3D) and unhosted (2D) run one path. First entry in the **Scripts** pulldown (Tools panel); no Core/VM split. See `Shim/Nudge/README.md` |
...
| `TurboSuite.Dmx` | TurboDMX — DMX-controlled RGBW LED tape/fixture automation (decoder/driver packing, addressing, one-line, wire legend, **one `<unnamed>` electrical circuit per Control Zone** — all the zone's DMX fixtures + decoders + drivers, so the Load Schedule totals the zone and TurboZones names it; a zone's several decoders are power subdivision cloned onto its one control address, hence one circuit not one-per-decoder — reconciled two-phase on re-Place in `DmxPlacementService.CircuitZones`, Control-Zone view overlay). **Gated behind `ExperimentalCommandsEnabled`; feature-complete + live-tested, end-of-project polish only.** Has its own `Shim/Dmx/README.md`. Pure engine + VMs in `Core/Dmx/` (unit-tested in `Tests/`); Revit-coupled half in `Shim/Dmx/`, loop-centric modeless window (`DmxMainViewModel`). State persists in one JSON-backed ExtensibleStorage schema (`DmxStorageService`). Design rationale + domain vocabulary live **in the code**: start at the canonical containment-ladder / overloaded-word glossary on `DmxSolver` (`Core/Dmx/DmxSolver.cs`), then the per-file doc-comments — there are no separate DMX design docs |
| `TurboSuite.Dali` | TurboDALI — standalone DALI addressing command: group Control Zones into **loops**, declare each loop's panel **ZONE**, and **write per-unit `L#-##` addresses** back onto the model — one per **addressable unit** (a driver device, or a self-driven downlight fixture; `##` is the **zero-based** DALI hardware short address 0–63), with a job-wide numbering lock. **Gated behind `ExperimentalCommandsEnabled`** (with TurboDMX); it is the **sole DALI editor** — the transitional TurboZones DALI tab was removed, so DALI declaration is **dev-only until this ungates**. TurboZones is now a pure consumer (demand via `DaliDemandProvider`, placement via `DaliPlacementMapper`). Pure engine + VMs in `Core/Dali/` (addressing/lock reconciler, NW-seeded `ProximityWalk`, unit-tested); Revit-coupled half in `Shim/Dali/` (modeless `DaliMainViewModel`). Rides the existing DALI schema (`DaliStorageService`, `PayloadVersion` v4 unit-anchors the lock snapshot; a pre-v4 circuit-anchored baseline is discarded on load, loops preserved). The addressing/counting grain is the addressable unit (shared `DaliUnitEnumerator` walk feeds both `DaliDemandProvider` and the addressing reader); the durable unit key is `circuit.UniqueId#ordinal` for a driver (ordinal = Switch ID suffix) / fixture UniqueId for a downlight, so addresses survive a driver redeploy. Has its own `Shim/Dali/README.md` |
| `TurboSuite.Snoop` | TurboSnoop — read-only, **selection-aware** reporter, one button two branches: **own element selected** → what it's hosted to, with a churn/orphan **risk tier** (`HostResolutionService` + the pure `Core/Host` resolution/classifier — resolves the linked host via `HostFace.LinkedElementId`, the identity path that survives where the geometric-face path failed); **nothing selected** → pick a linked family → **which VG checkbox to uncheck**. Deliberately **read-only, no Apply** on both branches. The full-model host audit is a deliberate stub (`ResolveAll`, home TBD) |
| `TurboSuite.Schedule` | TurboSchedule — page-per-Type-Mark form-view spec editor for lighting fixtures and drivers, unifying the two native spec schedules. Modeless; data-driven `FieldDef` roster, `ScheduleTypeCollector` reconciles each field across all symbols sharing a Type Mark. Also a per-project **Excel round-trip** (one **Sync workbook** button) so non-Revit designers author specs in `.xlsx`: pure Core halves — `ScheduleWorkbookIo` (ClosedXML read/write, row-per-type, sheet=Kind) + `ScheduleSyncPlanner` (workbook→model diff) reconciled by the shim `ScheduleWorkbookGateway`; workbook is source-of-truth for spec fields, removed types get a one-cycle red→purge grace. Path stored in its own ES schema (`WorkbookPathStorageService`) |
...
- Custom parameters by name: "Switch ID", "Scale Factor", "Linear Length", "Linear Power", "Power", "Sub-Driver Power", "Derating Factor" (TurboDriver: Percentage — applied only to the packing ceiling, never the sub-driver-count validity math), "Dimming Protocol" (the module-type signal for TurboZones/TurboDocs — resolved via `Core/Zones/Services/DimmingModuleResolver.cs`, whose map is **not** the identity: MLV → ELV module, WIFI rides no module, DMX is owned by a subsystem that counts its own hardware — TurboDMX, via `IControlSubsystemDemandProvider` — and DALI is now subsystem-owned too (its `LQSE2-1DALUNV-D` DIN-module count comes from designer-declared loops, reported job-wide by `DaliDemandProvider`; the module lands `DinSlot` in a dimming panel, placement via each loop's ZONE assignment declared in **TurboDALI**); it is **also the DMX/DALI subsystem-membership signal** — TurboDMX now routes fixture membership on `Dimming Protocol = DMX`, not `DMX Channels > 0`), "Control Zone" (protocol-agnostic instance param grouping fixtures into a control subsystem's zones — DMX and DALI; blank = unassigned), "Voltage", "Maximum Fixtures" (TurboDriver: `0` = no limit), "Remote Power Supply", "Load Classification Abbreviation" (**no longer read by TurboSuite** — the connector-level value it used to source module type from; the constant remains for native-Revit use), "Load Classification", "Circuit Naming", "Circuit Prefix", "Circuit Prefix Separator", "Orientation", "Angle", "Two Gang" and "Wireless" (TurboZones keypads/repeaters: **link math only** — gang doubles a device against a link's 99, wireless moves it to a Clear Connect link; neither reaches the BOM, which groups purely by catalog number), "Catalog Number1"–"Catalog Number6" and "Catalog Qty1"–"Catalog Qty6" (TurboDocs Counts: a length-token + cut-list/qty mini-grammar — semantics and edge cases are pinned by `Core/Docs/Services/CatalogLengthTokenResolver.cs` and its oracle tests; read those, don't reconstruct the rules. **A second consumer:** `Core/Zones/Services/CatalogSlotTally.cs` reuses `CatalogQtyParser`/`Evaluate` and nothing else from Counts, to order a control device's several parts — it rejects the `@ft`/`@in` stock-cut modes, which need a Linear Length no keypad has), "Data Sheet URL", "Manufacturer", "DALI Address" (TurboDALI write-back target: the `L#-##` label — `##` is the **zero-based** DALI hardware short address (0–63) — a **shared** instance param bound to Lighting Fixtures **and** Lighting Devices so the driver device carries it too — taggable, StorageType String; written **per addressable unit** onto its one element — the driver device or the self-driven fixture; a driver's tape fixtures carry none, stale values cleared). **Access via `TurboSuite.Shared.Constants.ParameterNames` — do NOT pass string literals to `LookupParameter`.**
...
- **Dispatch completions via a captured `Dispatcher`, never `Application.Current`**: Revit **2024** (the .NET Framework host) has **no WPF `Application`, so `System.Windows.Application.Current` is null** — `Application.Current?.Dispatcher?.BeginInvoke(...)` there is a **silent no-op** that drops every completion callback (2025/2026 have an Application, so this bites *only* on 2024 — which the `/debug` loop, building the 2025 shim, never exercises). It wedged TurboName hard: its single-shot `_eventBusy` latch resets only from a dispatched completion, so a dropped one left the window unable to act or close; `RevitWorkQueue` (TurboNumber/Zones/DMX/RPS) has no latch, so it merely lost its post-op UI refresh — same root cause. **Fix pattern:** capture `Dispatcher.CurrentDispatcher` at handler/queue construction (the command builds it on the API/UI thread — spike-confirmed thread id 1, the same thread the modeless window lives on) and marshal through that field, with a direct-invoke fallback when the dispatcher is null/shutting down. See `TurboNameApiHandler.Dispatch` and `RevitWorkQueue.Execute`.
