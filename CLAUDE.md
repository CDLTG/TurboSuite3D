...
Supporting these: version-agnostic, multi-targeted (`net48;net8.0-windows`) `Core/` and `Abstractions/` (no Revit refs), plus `Updater/` and `Installer/`. Tests live in `Tests/TurboSuite.Core.Tests.csproj` (xUnit, net8.0-windows; run `dotnet test`) — automated oracle suites for the pure, Revit-free logic in `Core/` (the `Core/Dmx/` engine, the `Core/Driver/` `StaleClassifier`, the `Core/Docs/` Counts catalog cut-list/qty engine, the `Core/Name/` region watershed + room-label grouping + ceiling-height rounding, and the `Core/Zones/` allocator/label/link services, expanding as more pure Core logic lands); the shims are validated by manual in-Revit testing. Core exposes internals to the test assembly via `<InternalsVisibleTo>` (compile-time only) so internal helpers can be pinned directly. No linting configs.
...
| `TurboSuite.Name` | TurboName — **modeless** 2D job-setup window (MVVM; one `IExternalEventHandler`). Folds in the linked-CAD layer list (VG → Imported Categories: live show/hide + `Find` + per-layer **Line Graphics** color/weight/pattern override via a native-style dialog, persisted on the view; **Ctrl/Shift multi-select** bulk-applies visibility, `W`/`D`/`A` tags and line edits — one list-carrying request per action, since the shared event drops extra raises; **Hide by picking** loops the native Query ▸ "Hide in view" — click geometry, its layer hides, Escape ends it, guarded so an unrecognized category id is refused rather than blanking the whole DWG), click-to-tag layer roles — `W`/`D`/`A` region-gen and `N`/`H` room-name/height text (Text mode); tagging is pure data, and a single **Preview watershed** toggle (off by default) paints all tagged layers red on demand via `SetCategoryOverrides` (reverted on toggle-off/close, `LayerRolePreviewService`). Then one **Generate Regions &amp; Names** group — Auto watershed / Rectangle / Polygon plus **Assign names** — all against the launch view. Room names + ceiling heights are scoped to their own link independently (`RoomNameLinkName`/`CeilingHeightLinkName`, blank = all links); region-gen entries are `file\|layer`-qualified (bare = legacy under `SourceLinkName`). The Revit-free scope matcher is `Core/Name/CadLinkScope` (unit-tested); this per-link scoping is what killed the plan+RCP double-seed. **Both shims enable `UseWindowsForms` (native `ColorDialog`) with those two WinForms global usings suppressed to avoid `View`/`Color` collisions.** |
...
| `TurboSuite.Mask` | TurboMask — masking region + per-fixture annotation stamps (from each family's nested Generic Annotation, loaded as `Stamp_*`) + detail-line overlays of wires connected to the masked devices. Real wires are left connected/hidden under the mask — overlays are view-only stand-ins, never a delete-recreate, so connectivity is untouched. Shipped |
| `TurboSuite.Setup` | TurboSetup — new-project setup: copy levels from the linked arch model, create Floor/RCP views with firm templates, configure RVT link display (**3D RVT-linked only**; link-graphics path is Revit 2025-only, 2024 sets up levels/views/templates and leaves links manual). Shipped |
| `TurboSuite.Dmx` | TurboDMX — DMX-controlled RGBW LED tape/fixture automation (decoder/driver packing, addressing, one-line). **Gated behind `ExperimentalCommandsEnabled`; feature-complete + live-tested, end-of-project polish only.** Pure engine + VMs in `Core/Dmx/` (unit-tested in `Tests/`); Revit-coupled half in `Shim/Dmx/`. Loop-centric modeless window (`DmxMainViewModel`): a zone pool of unassigned `Control Zone`s feeds a loops tree, with per-loop click-to-place (decoder+driver families), a Control-Zone-anchored numbering lock (`DmxLockReconciler`; REVIEW on interface reassignment only — a same-count decoder-type swap doesn't flag), per-zone cluster sub-builder (`DmxZoneBuilder`), and a per-loop view-owned one-line diagram. Fixtures that can only field-connect in daisy-chains (e.g. LED light sheets, max N per tap) declare a `DMX Bundle Size` type param (Integer); the input layer (`Core/Dmx/Input/DmxBundler.cs`) coalesces each chain into ONE indivisible `TapeRun` per cluster — grouped by `(Type Mark, channels, bundle size)`, sliced deterministically by ElementId — so the packer sizes decoders by whole chains, not individual fixtures. Default (missing/≤1) = one run per fixture (unchanged). The one-line/legend/lock are untouched (no per-fixture id flows downstream) (`Core/Dmx/OneLine/`, wipe-and-redraw). A per-job **wire legend** — dense, per-job numbering off an uncapped `#16-N` conductor engine with a job-wide pull-up (`DmxWireLegend`) — surfaces both as an in-window panel and a generated drafting view (`Legend view ▸`); the same numbers stamp every wire on the one-line. While the window is open, the active view's DMX fixtures are colored by `Control Zone` via per-element overrides (`Core/Dmx/Overlay/DmxZonePalette` + `Shim/Dmx/Services/DmxZoneColorService`), reverted on close. State persists in one JSON-backed ExtensibleStorage schema (`DmxStorageService`). The design rationale + domain vocabulary now live **in the code**: start at the canonical containment-ladder / overloaded-word glossary on `DmxSolver` (`Core/Dmx/DmxSolver.cs`), then the per-file doc-comments — there are no separate DMX design docs. |
| `TurboSuite.Snoop` | TurboSnoop — read-only "which VG checkbox do I uncheck?" reporter: pick a linked arch family, list the Visibility/Graphics **Category → Subcategory** checkboxes its geometry draws under (Model via one viewless `get_Geometry`; annotation via per-`ViewPlan` sweep + union). Deliberately **read-only, no Apply** — no API can flip the per-link VG checkbox, so it names the box and the user unchecks it (modeless, `ShowActivated=false`, keeps Revit's VG keybind live). Shipped. Design rationale in `Shim/Snoop/` headers |
| `TurboSuite.Schedule` | TurboSchedule — page-per-Type-Mark form-view spec editor for lighting fixtures (`OST_LightingFixtures`) and drivers (`OST_LightingDevices`), unifying the two native spec schedules. Modeless WPF; data-driven `FieldDef` roster filtered by `PageKind`. `ScheduleTypeCollector` reconciles each field across all symbols sharing a Type Mark into a `SpecField` (states: n/a / read-only / ⟨varies⟩ / normal). Core can't reference Revit `StorageType`, so it maps to a Core `SpecValueKind` for display while the writer re-reads the live type at save (Integer params set via `Parameter.Set(int)`, not `SetValueString`; unit-bearing Doubles via `AsValueString`). Explicit Save flushes dirty pages in one transaction. Shipped |
