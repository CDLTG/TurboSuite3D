...
TurboSuite is a unified Autodesk Revit add-in for electrical/lighting automation, written in C#, supporting **Revit 2024 and 2025**. A per-version `TurboSuite.dll` (.NET 8.0-windows for 2025, .NET Framework 4.8 for 2024) implements `IExternalApplication` and registers fourteen shipped commands (TurboDriver, TurboRPS, TurboName, TurboBubble, TurboTag, TurboWire, TurboZones, TurboNumber, TurboCompact, TurboTab, TurboDocs, TurboMask, TurboSnoop, TurboSchedule) plus a Settings dialog across three ribbon panels (Settings, Commands, Utilities). Two more commands, TurboSetup and TurboDMX, are compiled in but gated behind `ExperimentalCommandsEnabled` in `App/TurboSuiteApplication.cs` (TurboDMX is mid-build — its read-only window ships; model-writing phases pending).
...
| `TurboSuite.Dmx` | TurboDMX — DMX-controlled RGBW LED tape/fixture automation (decoder/driver packing, addressing, one-line). **Gated behind `ExperimentalCommandsEnabled`; mid-build.** Pure engine in `Core/Dmx/` (ported from a standalone spike, fully unit-tested in `Tests/`); Revit-coupled half in `Shim/Dmx/`. Phase 1 shipped: a modeless window (`DmxMainViewModel` + reader) reads DMX fixtures (`DMX Channels` > 0) grouped by the `Control Zone` instance param, discovers decoder/driver family types, and shows a live "bill" off the pure `DmxSolver` — **read-only, zero model writes**. Doc-side state (loops/clusters/control-system/settings/snapshot) persists in one JSON-backed ExtensibleStorage schema (`DmxStorageService`). Schema conventions centralized in `Core/Dmx/DmxSchemaAssumptions.cs`. Model writes (placement, `Switch ID`/`Type Mark` tagging), the numbering lock/snapshot, and one-line generation are pending phases. Design docs are local-only under `Specs/_DMX/` (gitignored) |
