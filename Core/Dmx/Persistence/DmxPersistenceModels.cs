...
namespace TurboSuite.Dmx.Persistence
{
    ...
    // overlays" above the read-only model + native Control Zone parameter — see TurboDMX).
    ...
    public sealed class DmxModuleState
    {
        ...
        /// <summary>Last solve snapshot (lock-aware re-run safety). Null until first solved.</summary>
        public DmxSnapshotDto? Snapshot { get; set; }

        /// <summary>Placement registry ( Option-A cleanup): every placed decoder's DEC # + its decoder
        ...
        /// <summary>One-line view registry: interface # → the owned Drafting View's element id, so a
        ...
        /// <summary>The single per-job wire-legend Drafting View's element id, so a re-draw finds +
    ...
    /// to the same sensible values the window opens with.</summary>
    public sealed class DmxSettingsDto
    {
        ...
        /// <summary>Job-wide homerun pull-up: stock sizes to bump every LV homerun past exact. 0 = exact.</summary>
        ...
        /// <summary>Curated decoder family types — the job's kit, ticked from discovery. Stored as
    ...
    public sealed class DmxLoopDto
    {
        ...
        /// <summary>Smart-fixture channels reserved off this loop's interface budget. 0 = none.</summary>
    ...
    /// <summary>Solve snapshot for re-run safety ( numbering lock). The lock baseline is
    ...
    {
        /// <summary>Numbering lifecycle. Persisted values: "Unlocked" / "Locked"
