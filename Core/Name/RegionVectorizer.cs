...
{
    /// <summary>
    /// Turns one owner's raster territory (from the <see cref="RegionWatershedEngine"/> flood grid)
    /// into a clean, wall-aligned boundary polygon ready for <c>FilledRegion.Create</c>. Pure geometry —
    /// no Revit. A pixel belongs to the owner iff <c>grid[i] == owner</c>; every other value
    ...
    public static class RegionVectorizer
    {
        ...
        /// coordinates produced by <paramref name="toModel"/>), or null if it can't be vectorized.
        /// </summary>
        public static List<Pt> Trace(int[] grid, int w, int h, int owner, long pixelCount,
            Func<int, int, Pt> toModel, List<WallSeg> walls)
            => Trace(grid, w, h, owner, pixelCount, toModel, walls, out _);

        /// <summary>
        /// As <see cref="Trace(int[],int,int,int,long,Func{int,int,Pt},List{WallSeg})"/>, but on a null
        ...
        /// </summary>
        public static List<Pt> Trace(int[] grid, int w, int h, int owner, long pixelCount,
            Func<int, int, Pt> toModel, List<WallSeg> walls, out string failReason)
        {
            failReason = null;
            ...
            var contour = px.Select(p => toModel(p.x, p.y)).ToList();
            ...
            // the un-aligned DP polygon sits ~1" off the wall face (breaks downstream keypad/boundary-nudge), and jog
            // flushing/squaring only ever produced quietly-wrong regions the user had to hand-clean anyway — an
            // honest, named skip beats a created-but-flawed region ("auto-gen only ever helps").
            //
            // Reverted repair attempts (do NOT rebuild — all tried, all worse than an honest skip; the code is
            // in `git log RegionVectorizer.cs` if ever revived): (1) in-place jog repair — v1 midpoint-collapse
            // left diagonals, v2 squaring was fragile on clustered jogs, v3 `DeJog` flush flattened EVERY jog to
            // fix one pinch; (2) global 24 px/ft resolution bump — fixed one small-feature room but captured
            // sub-inch noise elsewhere (dropped created 53→49) and made ppf floor-size-dependent via the pixel
            // cap ⇒ non-deterministic. A durable small-feature fix must be resolution-INDEPENDENT (adaptive
            // tolerance / vector-space handling), never a global ppf bump.
        ...
        {
            if (points.Count <= 3) return new List<Pt>(points);
            ...
            if (maxDist > tolerance)
            {
                ...
                var result = new List<Pt>(left);
                result.AddRange(right.Skip(1));
                return result;
            }

            return new List<Pt> { first, last };
        }
        ...
        private static List<Pt> AlignToWalls(List<Pt> poly, List<WallSeg> walls)
        {
            int n = poly.Count;
            var linePt = new Pt?[n];   // per-edge matched wall line: a point on it …
            var lineDir = new Pt?[n];  // … and its unit direction (null ⇒ edge unmatched)

            for (int i = 0; i < n; i++)
            {
                if (MatchWall(poly[i], poly[(i + 1) % n], walls, out Pt p, out Pt d))
                {
                    linePt[i] = p;
                    lineDir[i] = d;
                }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (mp && mc)
                {
                    if (AngleBetween(lineDir[prev].Value, lineDir[i].Value) > CornerParallel)
                    {
                        var x = IntersectLines(linePt[prev].Value, lineDir[prev].Value, linePt[i].Value, lineDir[i].Value);
                        v = (x != null && x.Value.DistanceTo(poly[i]) <= MaxCornerReach)
                            ? x.Value
                            : ProjectOnLine(poly[i], linePt[i].Value, lineDir[i].Value);
                    }
                    else
                    {
                        // Two near-parallel walls (a straight run DP split in two) — one line, just project.
                        v = ProjectOnLine(poly[i], linePt[i].Value, lineDir[i].Value);
                    }
                }
                else if (mc) v = ProjectOnLine(poly[i], linePt[i].Value, lineDir[i].Value);
                else if (mp) v = ProjectOnLine(poly[i], linePt[prev].Value, lineDir[prev].Value);
        ...
        private static bool MatchWall(Pt a, Pt b, List<WallSeg> walls, out Pt linePt, out Pt lineDir)
        {
            linePt = default;
            lineDir = default;
            bool matched = false;
            ...
            foreach (var seg in walls)
            {
                ...
                double dist = PerpDistance(mid, seg.Start, sdir);
                if (dist >= best) continue;
                if (!Overlaps(a, b, seg.Start, sdir, slen)) continue;
        ...
        private static bool Overlaps(Pt a, Pt b, Pt origin, Pt dir, double slen)
        {
        ...
        private static double AngleBetween(Pt u, Pt v)
        {
            double d = Math.Min(1.0, Math.Abs(u.X * v.X + u.Y * v.Y));
            return Math.Acos(d);
        }

        private static double PerpDistance(Pt p, Pt origin, Pt dir)
        {
            double along = (p - origin).X * dir.X + (p - origin).Y * dir.Y;
            var proj = new Pt(origin.X + along * dir.X, origin.Y + along * dir.Y);
            return p.DistanceTo(proj);
        }

        private static Pt ProjectOnLine(Pt p, Pt origin, Pt dir)
        {
            double along = (p - origin).X * dir.X + (p - origin).Y * dir.Y;
            return new Pt(origin.X + along * dir.X, origin.Y + along * dir.Y);
        }
        ...
        private static List<Pt> Cleanup(List<Pt> poly)
        {
            const double eps = 0.5 / 12.0; // 0.5"

            var dedup = new List<Pt>(poly.Count);
            ...
            var result = new List<Pt>(dedup.Count);
            int n = dedup.Count;
            for (int i = 0; i < n; i++)
            {
        ...
        private static string SelfIntersectionReason(List<Pt> poly)
        {
            int n = poly.Count;
            for (int i = 0; i < n; i++)
            {
        ...
        private static string Fmt(Pt p) => $"({p.X:F1}, {p.Y:F1})";
        ...
        private static double Cross(Pt a, Pt b, Pt c) =>
        ...
        private static Pt? IntersectLines(Pt p1, Pt d1, Pt p2, Pt d2)
        {
            ...
            return new Pt(p1.X + t * d1.X, p1.Y + t * d1.Y);
        }

        private static double PointToSegmentDistance(Pt p, Pt a, Pt b)
        {
            double dx = b.X - a.X;
            double dy = b.Y - a.Y;
